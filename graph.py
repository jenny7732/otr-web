...
def detailed_plot_pitch_from_sound_file(sound_file, target_length, color='g', label=None, window_size=25):
    # 음성 파일에서 pitch 추출
    ...
    # 그래프 그리기
    plt.plot(times_ms, smoothed_pitch_values, linewidth=5, color=color, label=label)

    return times_ms, smoothed_pitch_values

plt.figure(facecolor='#F9E0AE')

# 첫 번째 음성 파일 그래프 그리기 (길이를 10로 확대)
sound_file1 = 'static/audio/woctimnd/audio.wav'
times_ms1, smoothed_pitch_values1 = detailed_plot_pitch_from_sound_file(sound_file1, target_length=10, color='#FC8621', label='ORTOOREE')

# 두 번째 음성 파일 그래프 그리기 (길이를 10로 확대)
sound_file2 = 'static/audio/xarlaefr/audio.wav'
times_ms2, smoothed_pitch_values2 = detailed_plot_pitch_from_sound_file(sound_file2, target_length=10, color='#B14000', label='USER')
